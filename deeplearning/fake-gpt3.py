...
def build_reglaiour_model(max_index_up_text, maxium_train_up_legth):
    down_text_tensor = 0
    up_text = Input(shape=(None, maxium_train_up_legth), name='up_text')
    ...
    down_text_tensor = Input(shape=(None,maxium_train_up_legth),  name='down_text')

    down_text_tensor_emb = layers.Embedding(max_index_up_text, 64)(down_text_tensor)
    cnn_down_output_1 = layers.Conv1D(500,1,padding='same')(down_text_tensor_emb)
    cnn_down_output_2 = layers.Conv1D(500,2,padding = 'same')(down_text_tensor_emb)
    cnn_down_output_3 = layers.Conv1D(500,3, padding = 'same')(down_text_tensor_emb)

    cnn_down_output = layers.add([cnn_down_output_1, cnn_down_output_2, cnn_down_output_3])
    cnn_down_output = layers.Reshape((maxium_train_up_legth, 500))(cnn_down_output)
...
model = build_reglaiour_model(max_enc_index, maxium_train_up_legth)
print(model.summary())
...
model.fit({'up_text': train_up_arry, 'down_text': train_down_arry}, train_target_arry, verbose= 1 , callbacks= tensor_callback, epochs= 100, batch_size=1)
